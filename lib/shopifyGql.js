...
export const getAllProductsQuery = async () => {
  ...
  console.log(`%c response`, 'background: #0047ab; color: #fff; padding: 2px:', response)
...
export const getAllCurrentTags = async () => {
  ...
  const allCurrentTags = response.data.products.edges.map(
    item => item.node.tags.length && item.node.tags
  )
...
export const getAllItemsByTag = async tag => {
  const query = allProductsQuery
  const response = await callShopify(query)
  const allMatchingItems = response.data.products.edges.filter(item => item.node.tags.includes(tag)
  )
